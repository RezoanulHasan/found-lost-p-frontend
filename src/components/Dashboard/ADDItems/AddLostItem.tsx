...
import { AddLostItemInfo } from "./acton";
...
const AddLostItem: React.FC = () => {
  useTitle("Add Lost Item");
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data: TItemData) => {
    try {
      ...
      if (imageData && imageData.data && imageData.data.url) {
        ...
        const res = await AddLostItemInfo(formData);

        if (res.success) {
          Swal.fire({
            title: `Report Add Successful, ${data?.lostItemName}!`,
            text: res.message,
            icon: "success",
          }).then(() => {
            router.push("/allLostReport");
          });
        } else {
  ...
  return (
    <>
      <Container>
        <SectionTitle subHeading="ADD YOur lost Item " heading="Add Report" />
